...
/// FineWeb-2 込みの Kneser-Ney の bigram の上で 0 から 4 を振り、自分の文で
/// 最も良かった 3 にした（Wikipedia では 1 より 4 ポイント低い）。
pub const RANK_WEIGHT: f64 = 3.0;

/// 読みをそのままかなにした候補に加えるコスト。大文字で始めた
/// セグメントは「ここを変換したい」という指示なので、かなのままは
/// 辞書候補より不利にする。0 から 8 を振り、自分の文で最も良かった 4 にした
/// （6 では別の文が落ちる）。
