...
確定した文をエンジンに送ると、その文で `lm.zst` の出力層（最後の線形層の重みと
bias）を 1 歩動かす。動かすのは出力層だけで、他の層と `model.zst`、辞書は変わらない。
誤変換かどうかは見ないので、送った文はそのまま学習に回る。歩幅は `--adapt-lr`
（既定 3e-3）で決める。学習は変換とは別のスレッドで走り、変換の応答は待たせない。

動いた出力層はエンジンの終了時に `sekken-server-adapted-lm`（既定は
