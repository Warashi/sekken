...
動作は妨げない。語を伸ばす・縮める打鍵では置き換わらない。`.` `,` などの記号も語の
文字なので、日本語の文は改行まで 1 語として扱われ、改行で一括して置き換わる。
後退削除を別のコマンドに割り当てていれば `sekken-live-continue-commands` に足す。
足さないと、そのキーを押した時点で語が置き換わってから削除が走る。
...
- 置き換わった結果が英字で終わっても（`kyouha'Emacs` → `きょうはEmacs`）、その英字は次の語に含まれない。置き換わった文字を削除・編集すると、そこからは普通の語として扱う
