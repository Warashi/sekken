...
文脈によって別の語が勝つことはある。登録は、その読みを登録した語に確定した 1 回分として
個人化（次の節）にも学習させる。外れた語で置き換わった文を個人化が学習した後でも、
単独や助詞付きで打てば次の変換から登録した語が先に出る。文の中では外れた語が勝つことが
残り、その場合は登録した語を `C-j` で一度選べば次から先に出る。

### 個人化

エンジンの候補で語が確定するたび（語の終わりで 1 位候補に置き換わったとき、
`C-j` で候補を選んだとき）、その読みと文をエンジンに送り、`lm.zst` の出力層
