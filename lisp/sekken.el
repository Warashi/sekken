...
(define-minor-mode sekken-mode
  ...
  (if sekken-mode
      (progn
        ...
        (add-hook 'before-change-functions #'sekken-word-before-change nil t)
        (add-hook 'after-change-functions #'sekken-word-after-change nil t))
    (remove-hook 'pre-command-hook #'sekken-live-before-command t)
    (remove-hook 'post-command-hook #'sekken-live-after-command t)
    (remove-hook 'before-change-functions #'sekken-word-before-change t)
    (remove-hook 'after-change-functions #'sekken-word-after-change t)
    (sekken-word-reset)
